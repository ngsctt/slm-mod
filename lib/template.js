...
class Template {
  ...
  compile = function (src, options, vm) {
    ...
    if (options?.partials || this.partials) {
      const partials = Object.assign({}, this.partials, options.partials);
      for (const name of Object.keys(partials)) {  
        vm.cache(name, this.exec(partials[name], options, vm));
      }
    }
  ...
  exec = function (src, options, vm) {
    ...
    return vm.runInContext(this.src(src, options))[0];
  };
