...
class Parser {
  constructor (attrDelims) {
    this._attrDelims = attrDelims || { '(': ')', '[': ']' };

    let attrDelimsStr = '';

    for (const key of Object.keys(this._attrDelims)) {
  ...
  _pushOnTop = function (item) {
    this._stacks._last().push(item);
  };

  _sliceLine = function (beginSlice) {
  ...
  _matchLineThen = function (regex, next) {
  ...
  _ifTrueThen = function (condition, next) {
    if (condition) {
      next(this);
      return true;
    }
  ...
  _parseShortcutAttributes = function () {
  ...
  _parseTag = function (tag) {
    ...
    while (true) {
      ...
      if (m) {
        this._sliceLine(m[0].length);
        const trailingWS2 = m[2].indexOf('>') !== -1;
  ...
  _parseTextBlock = function (firstLine, textIndent, inTag) {
    const result = ['multi'];
  ...
  _parseJSCode = function (outerDelimeter) {
    let code = '';
    let count = 0;
    let delimiter;
    let closeDelimiter, m;
  ...
  _parseQuotedAttribute = function (quote) {
    let value = '', count = 0;
...
const _parseHtmlComment = function (parser, m) {
...
const _parseHtmlConditionalComment = function (parser, m) {
  const block = ['multi'];
...
const _parseTextBlock = function (parser, m) {
  let char, space;
...
const _parseOutputBlock = function (parser, m) {
  // We expect the line to be broken or the next line to be indented.
  parser._sliceLine(m[0].length);

  const trailingWS = m[2].indexOf('>') !== -1;
  const block = ['multi'];
...
const _parseInlineHtml = function (parser) {
  const block = ['multi'];
...
const _parseCodeBlock = function (parser) {
  // We expect the line to be broken or the next line to be indented.
  parser._sliceLine(1);
  const block = ['multi'];
